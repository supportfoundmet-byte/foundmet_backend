...
export function mapConnectionState(connection, currentUserId) {
  ...
  if (connection.status === "pending") {
    return String(connection.fromUser?._id || connection.fromUser) ===
    ...
    if (existing) {
      ...
      if (existing.status === "blocked") {
        return sendError(
          res,
          403,
          "You cannot connect with this founder.",
          "BLOCKED",
        );
      }
  ...
  try {
    ...
    const connections = await ConnectionModel.find({
      ...
      .populate({
        path: "fromUser",
        select:
          "name role address photo projectDetails projectStatus phoneNumber allowPhoneRequest location.city",
        match: {
          isSuperAdmin: { $ne: true },
          isBlocked: { $ne: true },
          isDeleted: { $ne: true },
    ...
    const visibleConnections = connections.filter(
      (connection) => connection.fromUser && connection.toUser,
    );
    ...
    const receivedRequests = visibleConnections.filter(
      (item) =>
        String(item.toUser?._id) === String(userId) &&
        item.status === "pending",
    );
    const connected = visibleConnections.filter(
      (item) => item.status === "accepted",
    );
    const blocked = visibleConnections.filter(
      (item) => item.status === "blocked",
    );
    const states = {};
    visibleConnections.forEach((connection) => {
      const other =
        String(connection.fromUser?._id) === String(userId)
          ? connection.toUser
          : connection.fromUser;
      if (other?._id) {
...
export async function respondConnectionRequest(req, res) {
  try {
    ...
    return sendError(
      res,
      500,
      "Unable to update this connection request.",
      "INTERNAL_ERROR",
    );
  }
}
